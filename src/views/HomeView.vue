<template>
  ...
  <div class="p-3 relative" style="top: -56px">
    ...
    <div class="h-screen">
      <div id="map" class="h-8/12 rounded-lg shadow-md"></div>
    </div>
  </div>
  ...
  >
    <div class="p-3">
      <img src="@/assets/image/station.png" class="w-16 h-16 mx-auto mb-4" />
      <p class="text-gray-700 text-md text-center mb-3">
        {{ dialogStation.title }}
      </p>
...
const fetchMarker = async () => {
  if (map.getZoom() >= 13) {
    ...
    var stationMarker = L.icon({
      iconUrl: new URL('@/assets/image/station-marker.png', import.meta.url).href,
      iconSize: [32, 32],
      iconAnchor: [16, 32],
      popupAnchor: [0, -32],
    });
